...
\noindent Har fået udsættelse pga. undervisning i kurset Cryptography
...
En finke har én {\tt Behavior} (fremover kaldet finkens adfærd). Det
er adfærdens ansvar at afgøre om finken vil hjælpe en given anden
finke eller ej, ligesom adfærden underrettes om hvorledes andre
...
Finkerne i simulationen styres via et objekt baseret på {\em
  Strategy}-designmønstret. Interfacet {\tt Behavior} er en
specifikation af en finkes adfærd i forbindelse med møder med andre
finker, og hvert finke-objekt har en instans af en underklasse af {\tt
  Behavior}, som benyttes til at træffe beslutninger om finken skal
gøre i et møde.
...
overskrives så alle {\tt Behavior}s af samme type giver samme {\tt
...
Det består som udgangspunkt af en instans af {\tt BiotopeViewer}. Dette
...
til en {\tt Observer} og lod dette kalde tegne-metoder på {\tt
  AreaPanel}.  Dette har vi dog afviget fra og lavet {\tt AreaPanel}
selv til en {\tt Observer} som sørger for at blive opdateret når
biotopen ændres. Dette gør at det på et senere tidspunkt bliver
nemmere at ændre på hvordan biotopen bliver vist. Hvis man eksempelvis
er mere interesseret i hvor mange der er af de forskellige arter, så
...
derefter uafhængigt af {\tt BiotopeViewer}, og kan med meget få
...
Vi har gjort det muligt selv at indsætte og fjerne finker fra
biotopen vha. musen, det er også kontrollen der ændrer biotopen når
dette skal ske.
...
{\tt StatisticsPanel} fungerer på stort set samme måde som {\tt
BiotopeLogger}, men da det her skal vises i et Java-vindue bliver vi
nødt til at bruge nogle Java-komponenter til at vise teksten. Den
fundamentale forskel på {\tt
...
afvigelser som er nævnt andetsteds i rapporten.
...
nødvendigvis har bund i de konkrete adfærdsmønstre. Derfor har vi
valgt ikke at implementere bevægelse af eller mutation blandt finkerne.
...
Mutation ville gøre det besværligt at undersøge hvordan et enkelt
adfærdsmønster klarer sig. Vi kunne let implementere mutation i den grad
at en finke ind imellem fik afkom tilhørende en tilfældigt valgt anden
finketype, men det mener vi ikke ville
tilføje et nyt interessant element til simulationen.
